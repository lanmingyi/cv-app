<template>
	<view class="container" :class="[vtheme]">
		<view class="tab-content"  v-if="cardList.length>0">
				...
				<view class="cu-item shadow ">
					<view class="cu-list menu"  @tap="goDetail(item)">
						...
						<view class="cu-item">
							<view class="content">
								<text class="cuIcon-btn text-green"></text>
								<text class="text-grey">请假人</text>
							</view>
							<view class="action">
								<text class="">{{ item.creatorId }}</text> 
							</view>
						</view>
						<view class="cu-item">
							<view class="content">
								<text class="cuIcon-btn text-green"></text>
								<text class="text-grey">天数</text>
							</view>
							<view class="action">
								<text class="">{{ item.days }}</text> 
							</view>
						</view>
						<view class="cu-item">
							<view class="content">
						...
						<view class="cu-item">
							...
							<view class="action">
								<text class="">{{ item.beginTime }}--{{ item.endTime }}</text>
							</view>
						</view>
					</view>
...
export default {
	...
	methods: {
		...
		getOaTaskBase: function() {
			let url = ''
			this.$http
				.post('/workflow/oaLeave/getPageSet', {pageNo:this.pages,
					pageSize:10}, {
					params: {},
				})
				.then(res => {
		...
		edit: function(e) {
			...
			uni.navigateTo({
				url: 'leaveEdit?detail=' + JSON.stringify(e)
			});
		},
		goDetail: function(e) {
			//创建绑定数据
