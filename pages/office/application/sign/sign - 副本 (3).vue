<template>
	<view class="container" :class="[vtheme]">
		<view class="wrapper bg-box" @click="showColor = false">
			<view class="handRight flex align-center justify-between">
				<view class="handTitle font-main">电子签名</view>
				<view >
			...
			<view class="padding-tb-xs margin-lr flex align-center text-one-hidden font-second" v-if="filePath" v-clipboard:copy="filePath" v-clipboard:success="onCopy" v-clipboard:error="onError">
			...
			subCanvas() {
				this.ctx.draw(true, () => {
					uni.canvasToTempFilePath({
						...
						quality: 1, //图片质量
						success(res) {
							console.log(res.tempFilePath,res, 'canvas生成图片地址');
							uni.showToast({
								title: '已保存'
							});
							//保存到系统相册
