		...
		<view class="margin-bottom-max bg-box">
			<u-form :model="taskData" :rules="rules" ref="uForm" :errorType="errorType">
				<view class="padding-lr">
					...
					<u-form-item label-width="180" label-position="top" label="计划时间" :required="true">
						<u-form-item label-width="0" label-position="left" label=" " prop="planStartTime" class="border-bottom-none" style="padding: 0">
							<u-input :border="false" type="select" v-model="taskData.planStartTime" placeholder="计划开始时间" @click="()=>{showTime=true,showTimeType=1}" disabled iconName="" iconNameColor="cu-color-icon"></u-input>
						</u-form-item>
						<view class="border-center"></view>
						<u-form-item label-width="60" label-position="left" label=" " prop="planEndTime" class="border-bottom-none" style="padding: 0">
							<u-input :border="false" type="select" v-model="taskData.planEndTime" placeholder="计划结束时间" @click="()=>{showTime=true,showTimeType=2}" disabled iconName="" iconNameColor="cu-color-icon"></u-input>
						</u-form-item>
					</u-form-item>
					
					<u-form-item label-width="180" label-position="top" label="实际时间">
						<u-form-item label-width="0" label-position="left" label=" " prop="actualStartTime" class="border-bottom-none" style="padding: 0">
							<u-input :border="false" type="select" v-model="taskData.actualStartTime" placeholder="实际开始时间" @click="()=>{showTime=true,showTimeType=3}" disabled iconName="" iconNameColor="cu-color-icon"></u-input>
						</u-form-item>
						<view class="border-center"></view>
						<u-form-item label-width="60" label-position="left" label=" " prop="actualEndTime" class="border-bottom-none" style="padding: 0">
							<u-input :border="false" type="select" v-model="taskData.actualEndTime" placeholder="实际结束时间" @click="()=>{showTime=true,showTimeType=4}" disabled iconName="" iconNameColor="cu-color-icon"></u-input>
						</u-form-item>
					</u-form-item>
			...
			formSubmit(){
				...
				that.$refs.uForm.validate(valid => {
					if (valid) {
						...
						that.$http.post('/office/oaTaskBase/saveSon', that.taskData, {
								params: {},
							})
							.then(res => {
