<template>
	<view class="u-dropdown u-dropdown-getwidth" :style="dropDownShow ? '' :'overflow:hidden'">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getContentLeft() {
				let windowWidth = this.$u.sys().windowWidth;
				let result = await this.$u.getRect('.u-dropdown-getwidth');
				if (this.itemWidth) {
					this.$uGetRect('.u-dropdown__content').then(res => {
						// this.contentLeft = '-' + Number(result.left + 6)
						this.posAbsolute = 'fixed'
					})
				}
			},
		}
