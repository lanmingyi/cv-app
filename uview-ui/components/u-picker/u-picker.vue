 ...
 * @property {String} minuteStep 模式选择，mode为time的模式下  minute步长规则 默认为1
...
export default {
	...
	methods: {
		...
		setMinutes() {
			this.minutes = this.generateArray(0, 59, this.minuteStep);
		...
		getResult(event = null) {
			...
			if (this.mode == 'time') {
				...
				if(this.minuteStep>1){
					if (this.minute > this.minuteStep) {
						this.hour++;
						this.minute = '00'
					} else if (this.minute == '00') {
						this.minute = '00'
					} else if (this.minute <=this.minuteStep) {
