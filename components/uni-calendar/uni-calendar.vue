			...
			<view class="uni-calendar__wrapper">
                ...
                canlender.lunar.gzYear +
                  ...
                  '年)'
					...
					<uni-calendar-item v-if="slide === 'none'" :canlender="canlender" :lunar="lunar"
						@selectDays="selectDays" />
					<swiper v-else :style="{ height: domHeihgt + 'px' }" :current="currentIndex"
						:vertical="slide === 'vertical' ? true : false" :duration="duration" class="uni-calendar__body"
						circular skip-hidden-item-layout @animationfinish="animationfinish" @change="change">
	...
	export default {
		...
		methods: {
			...
			selectDays(params) {
				...
				// console.log(isDay);
				let month =
					canlender.weeks[week][index].month < 10 ? '0' + canlender.weeks[week][index].month : canlender.weeks[
						week][index].month
				...
				this.isSilde = true
				this.isClick = true
				console.log("params",params,canlender)
