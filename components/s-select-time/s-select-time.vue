    ...
    <view class="time-container">
        <view class="item-container" v-for="(item, index) in timeList" :key="index">
            <view class="time-item" @click="selectTime(item)" :class="{
                ...
                'start-time': true,
                'end-time': item.end,
								'isReserve-time': item.isReserve,
            }">
                <view>{{item.time}}</view>
								<view style="font-size: 12px;">{{item.isReserve?'已预约':(item.disabled?'已超时':'可预约')}}</view>
            </view>
...
export default {
    ...
    props: {
        ...
        isDisabled: {
            ...
            type: Array,
            default: () => [
							{startTime :'07:00',endTime :'08:30'},{startTime :'15:30',endTime:'16:30'}]
        },
    },
    data () {
        return {
    ...
    methods: {
        ...
        selectTime (item) { // 选择时间
				...
				changeTime(){
					...
					const arr = []
					if (moment(`${that.currentDate} ${timeStrat}`) > moment(`${that.currentDate} ${timeEnd}`)) {
					    arr[0] = timeEnd
					    arr[1] = timeStrat
					} else {
					    arr[0] = timeStrat
					...
					console.log("reserveList-----",that.reserveList)
					that.timeList.forEach(items=>{
						...
						if (moment(`${that.currentDate} ${items.time}`) > moment(`${that.currentDate} ${arr[0]}`) && moment(`${that.currentDate} ${items.time}`) < moment(`${that.currentDate} ${arr[1]}`)) {
						    items.range = true
						    items.rangeHelp = true
						} else if (items.time === arr[0]) {
						    items.rangeHelp = true
						} else {
						    items.range = false
						    items.rangeHelp = false
						}
						that.reserveList.forEach(e =>{
							if(moment(`${this.currentDate} ${items.time}`) >= moment(`${this.currentDate} ${e.startTime}`) && moment(`${this.currentDate} ${items.time}`) <= moment(`${this.currentDate} ${e.endTime}`)){
								// console.log("items----",items)
								items.isReserve = true
							}
						})
					})
