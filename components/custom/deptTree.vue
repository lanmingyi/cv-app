	...
	<view :style="{'height': height,'overflow': 'scroll'}"  class="pos-relative bg-box" :class="vtheme">
		...
		<view class="bg-box">
			<view class="container-list" v-if="tree.length > 0">
				<view class="common font-main" v-for="(item, index) in tree" :key="index">
					<label class="content">
						<!-- 多选选中 -->
						<view class="flex align-center content-box" @click.stop="clickChange(item,index)">
							<view v-if="props.showIcon == 'show'">
								<view class="checkbox class1" v-if="isCheck&&props.multiple">
								...
								<view class="checkbox class2" v-if="isCheck&&!props.multiple&&props.nodes" >
			...
			<view v-else class="empty text-center">
				<u-empty text="暂无子部门" mode="list" icon-size="200"></u-empty>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			clickChange: function(item,index){
				if(this.isCheck && this.props.multiple){
			...
			getOrganizeList(item){
				...
				that.$http
					.post(that.url.expandUrl, params, {
						params: {}
					})
					.then(res => {
						let list = res.data
