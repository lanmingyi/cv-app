				...
				<scroll-view :style="[scrollStyle]" scroll-y scroll-x>
					<view class="scroll-x-box">
						<u-tr class="u-tr scroll-x-top" :style="{ 'background-color' : columnBgColor}">
							<u-th 
								class="u-th" 
								:class="[column.fixed && column.fixed == 'left' ? 'scroll-x-left' : '',column.fixed && column.fixed == 'right' ? 'scroll-x-right' : '']"
								v-for="(column,columnIndex) in columnsList" 
								:width="column.width"
								:style="{'width':column.width,'border-left': column.fixed && column.fixed == 'right' ? `solid 1px ${borderColor}` : ''}"  
								:key="columnIndex">
					...
					<!-- 数据循环 -->
					<block v-else>
						<u-tr class="u-tr scroll-x-top" v-for="(data,dataIndex) in loadDataList" :key="dataIndex">
							<u-td class="u-td scroll-x-pos" 
								v-for="(item,index) in columnsList" 
								:class="[item.fixed && item.fixed == 'left' ? 'scroll-x-left' : '',item.fixed && item.fixed == 'right' ? 'scroll-x-right' : '']"
								:width="item.width" 
								:style="{'width':item.width,'border-left': item.fixed && item.fixed == 'right' ? `solid 1px ${borderColor}` : ''}"  
								:key="index">
