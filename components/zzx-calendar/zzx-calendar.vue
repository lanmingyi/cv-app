<template>
	<view class="zzx-calendar">
		<view class="calendar-heander font-main">
<!-- 			<view style="text-align: left;padding: 20rpx 0 30rpx 30rpx;font-weight: bold;">
		...
		<view class="calendar-weeks font-main">
					...
					<view class="calendar-days">
						<template v-if="sitem === current">
							<view class="calendar-day" v-for="(item,index) in days" :key="index" :class="{
								...
								<view class="dot-show" v-if="item.info"  :class="item.info == 1 ? 'bg-blue' : (item.info == 2 ? 'bg-orange' : '')">
								</view>
								<!-- :style="dotStyle" -->
							</view>
						</template>
			...
			initDate(cur) {
				...
				days.forEach(day => {
					...
					if(this.type == 'attendance'){
						if(day.time.getDay() != 0 && day.time.getDay() != 6){
							if(day.time.getDay() % 2 == 0){
								day.info = 2;
							} else {
								day.info = 1;
							}
	...
	.zzx-calendar {
		...
		.mode-change {
			...
			.mode-arrow-top {
				...
				border-bottom: 10upx solid $custom-font-main;
			}
			...
			.mode-arrow-bottom {
				...
				border-left: 12upx solid transparent;
				border-right: 12upx solid transparent;
				border-top: 10upx solid $custom-font-main;
			}
		}

		.is-today {
			background: #e7e7e7;
			// border: 1upx solid $theme-head-box-color;
			border-radius: 50%;
		}

		.is-checked {
			background: $custom-font-main !important;
			color: #ffffff;
		}
