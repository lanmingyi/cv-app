...
const successCode = 200 // 成功的code
const tokenExpireCode = 100 // token 过期的code (举例)
...
//   config.baseUrl = 'http://demo.misboot.com' /* 根域名不同 */
//   return config
// })
...
http.setConfig((config) => {
	/* 设置全局配置 */
	config.baseUrl = baseUrl /* 根域名不同 */ //http://vue.misboot.com:9300
	config.header = {
		...config.header,
	}
	// console.log(config.custom,uni.getStorageSync('token'))
...
http.interceptor.response(async (response) => {
	...
	if (response.data.code === tokenExpireCode) {
		...
		const currToken = uni.getStorageSync('token') // todo 获取token逻辑，可自定义
		if (expireToken.includes(currToken)) { // 本地储存的是过期token了，重新获取
			...
			if (getTokenResult === 'success') { // 获取新的token成功
				const repeatRes = currToken // XXX 可根据自身逻辑决定是否需要重新创建一个示例
